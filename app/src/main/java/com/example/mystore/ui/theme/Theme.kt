...
private val LightColorPalette = MyStoreColors(
    brand = Shadow5,
...
private val DarkColorPalette = MyStoreColors(
    brand = Shadow1,
...
object MyStoreTheme {
    val colors: MyStoreColors
        @Composable
        get() = LocalMyStoreColors.current
}

/**
 * MyStore custom Color Palette
 */
@Stable
class MyStoreColors(
    ...
    fun update(other: MyStoreColors) {
...
@Composable
fun ProvideMyStoreColors(
    colors: MyStoreColors,
    content: @Composable () -> Unit
) {
    val colorPalette = remember { colors }
    colorPalette.update(colors)
    CompositionLocalProvider(LocalMyStoreColors provides colorPalette, content = content)
}

private val LocalMyStoreColors = staticCompositionLocalOf<MyStoreColors> {
    error("No MyStoreColorPalette provided")
}

/**
 * A Material [Colors] implementation which sets all colors to [debugColor] to discourage usage of
 * [MaterialTheme.colors] in preference to [MyStoreTheme.colors].
 */
fun debugColors(
    darkTheme: Boolean,
